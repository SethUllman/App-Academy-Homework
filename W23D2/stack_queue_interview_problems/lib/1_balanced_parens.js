...
function balancedParens(str) {
  const leftPar = str.split('(');
  const rightPar = str.split(')');

  const leftBra = str.split('[');
  const rightBra = str.split(']');

  let count = 0;
  let leftCount = 0;
  while (count <= str.length){
    if (str[count] === ')' && leftCount === 0){
      return false;
    }
    if (str[count] === '('){
      leftCount++;
  ...
  if ((leftPar.length === rightPar.length) || (leftBra.length === rightBra.length)){
    return true;
  }
