...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    SELECT
      result_toys.name, result_toys.price 
    FROM
      toys AS shiny_mouse_toys
    JOIN
      toys AS result_toys ON result_toys.price = shiny_mouse_toys.price
    WHERE
      shiny_mouse_toys.name = 'Shiny Mouse' 
      AND result_toys.name != 'Shiny Mouse'
    ORDER BY
      result_toys.name
...
def just_like_orange
  ...
  execute(<<-SQL)
    SELECT
      result_cats.name, result_cats.breed
    FROM
      cats AS orange_cats
    JOIN
      cats AS result_cats
    ON 
      orange_cats.breed = result_cats.breed
    WHERE
      orange_cats.name = 'Orange' AND result_cats.name != 'Orange'
    ORDER BY
      result_cats.name
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT cats_2.name, toys_1.name
    FROM 
      cats AS cats_1
    JOIN
      cattoys AS ct1 ON ct1.cat_id = cats_1.id
    JOIN
      toys AS toys_1 ON ct1.toy_id = toys_1.id
    JOIN
      cattoys AS ct2 ON ct2.toy_id = toys_1.id
    JOIN
      cats AS cats_2 ON ct2.cat_id = cats_2.id
    WHERE
      cats_1.name = 'Jet' AND cats_2.name != 'Jet' 
    ORDER BY
      cats_2.name
  SQL
end
